...
import {carbon2185} from '../config.js';
...
export default class Actor5e extends Actor {
  ...
  prepareData() {
    ...
    // Skill modifiers
    const feats = carbon2185.characterFlags;
  ...
  _computeSpellcastingProgression (actorData) {
    ...
    const slots = carbon2185.SPELL_SLOT_TABLE[levels - 1] || [];
   ...
   */
  getLevelExp(level) {
    const levels = CONFIG.carbon2185.CHARACTER_EXP_LEVELS;
  ...
  getCRExp(cr) {
    ...
    return CONFIG.carbon2185.CR_EXP_LEVELS[cr];
  }
  ...
  async update(data, options={}) {
    ...
    if ( newSize && (newSize !== getProperty(this.data, "data.traits.size")) ) {
      let size = CONFIG.carbon2185.tokenSizes[newSize];
  ...
  async useSpell(item, {configureDialog=true}={}) {
    ...
    const usesSlots = (lvl > 0) && CONFIG.carbon2185.spellUpcastModes.includes(itemData.preparation.mode);
    ...
    if ( limitedUses ) {
      ...
      if ( uses <= 0 ) ui.notifications.warn(game.i18n.format("carbon2185.ItemNoUses", {name: item.name}));
  ...
  rollSkill(skillId, options={}) {
    ...
    return d20Roll(mergeObject(options, {
      parts: parts,
      data: data,
      title: game.i18n.format("carbon2185.SkillPromptTitle", {skill: CONFIG.carbon2185.skills[skillId]}),
  ...
  rollAbility(abilityId, options={}) {
    const label = CONFIG.carbon2185.abilities[abilityId];
    new Dialog({
      title: game.i18n.format("carbon2185.AbilityPromptTitle", {ability: label}),
      content: `<p>${game.i18n.format("carbon2185.AbilityPromptText", {ability: label})}</p>`,
      buttons: {
        test: {
          label: game.i18n.localize("carbon2185.ActionAbil"),
          callback: () => this.rollAbilityTest(abilityId, options)
        },
        save: {
          label: game.i18n.localize("carbon2185.ActionSave"),
  ...
  rollAbilityTest(abilityId, options={}) {
    ...
    if ( feats.remarkableAthlete && carbon2185.characterFlags.remarkableAthlete.abilities.includes(abilityId) ) {
  ...
  rollAbilitySave(abilityId, options={}) {
    ...
    return d20Roll(mergeObject(options, {
      parts: parts,
      data: data,
      title: game.i18n.format("carbon2185.SavePromptTitle", {ability: label}),
      speaker: ChatMessage.getSpeaker({actor: this}),
      halflingLucky: this.getFlag("carbon2185", "halflingLucky")
    }));
  }
  ...
  async rollDeathSave(options={}) {
    ...
    const roll = await d20Roll(mergeObject(options, {
      parts: parts,
      data: data,
      title: game.i18n.localize("carbon2185.DeathSavingThrow"),
      speaker: speaker,
      halflingLucky: this.getFlag("carbon2185", "halflingLucky"),
      targetValue: 10
    }));
    ...
    if ( success ) {
      ...
      if ( roll.total === 20 ) {
        ...
        await ChatMessage.create({content: game.i18n.format("carbon2185.DeathSaveCriticalSuccess", {name: this.name}), speaker});
      }
      ...
      // 3 Successes = survive and reset checks
        ...
        await ChatMessage.create({content: game.i18n.format("carbon2185.DeathSaveSuccess", {name: this.name}), speaker});
      }
    ...
    // Save failure
      ...
      if ( failures === 3 ) {
        ...
        await ChatMessage.create({content: game.i18n.format("carbon2185.DeathSaveFailure", {name: this.name}), speaker});
      }
  ...
  async rollHitDie(formula) {
    ...
    if ( !cls ) {
      return ui.notifications.error(game.i18n.format("carbon2185.HitDiceWarn", {name: this.name, formula: formula}));
    }

    // Prepare roll data
    const parts = [formula, "@abilities.con.mod"];
    const title = game.i18n.localize("carbon2185.HitDiceRoll");
  ...
  async shortRest({dialog=true, chat=true}={}) {
    ...
    if ( chat ) {
      let msg = game.i18n.format("carbon2185.ShortRestResult", {name: this.name, dice: -dhd, health: dhp});
      ChatMessage.create({
        user: game.user._id,
  ...
  async longRest({dialog=true, chat=true}={}) {
    ...
    if ( chat ) {
      ChatMessage.create({
        user: game.user._id,
        speaker: {actor: this, alias: this.name},
        flavor: game.i18n.localize(newDay ? "carbon2185.OvernightRest" : "carbon2185.LongRest"),
        content: game.i18n.format("carbon2185.LongRestResult", {name: this.name, health: dhp, dice: dhd})
      });
    }
  ...
  async transformInto(target, { keepPhysical=false, keepMental=false, keepSaves=false, keepSkills=false,
    ...
    if ( !allowed && !game.user.isGM ) {
      return ui.notifications.warn(game.i18n.localize("carbon2185.PolymorphWarn"));
    }
    ...
    for ( let k of Object.keys(abilities) ) {
      ...
      else if ( keepMental && ["int", "tech", "people"].includes(k) ) abilities[k] = oa;
    ...
    if (!keepClass && d.data.details.cr) {
      d.items.push({
        type: 'class',
        name: game.i18n.localize('carbon2185.PolymorphTmpClass'),
  ...
  async revertOriginalForm() {
    ...
    if ( !this.owner ) {
      return ui.notifications.warn(game.i18n.localize("carbon2185.PolymorphRevertWarn"));
    }
  ...
  static addDirectoryContextOptions(html, entryOptions) {
    entryOptions.push({
      name: 'carbon2185.PolymorphRestoreTransformation',
