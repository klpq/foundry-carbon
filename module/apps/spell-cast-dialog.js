...
export default class SpellCastDialog extends Dialog {
  constructor(actor, item, dialogData={}, options={}) {
    ...
    this.options.classes = ["carbon2185", "dialog"];
  ...
  static async create(actor, item) {
    ...
    const lvl = id.level;
    const canUpcast = (lvl > 0) && CONFIG.carbon2185.spellUpcastModes.includes(id.preparation.mode);
    ...
    const spellLevels = Array.fromRange(10).reduce((arr, i) => {
      ...
      arr.push({
        level: i,
        label: i > 0 ? `${CONFIG.carbon2185.spellLevels[i]} (${slots} Slots)` : CONFIG.carbon2185.spellLevels[i],
    ...
    if (pact.level >= lvl) {
      ...
      spellLevels.push({
        level: 'pact',
        label: game.i18n.localize('carbon2185.SpellLevelPact')
          + ` (${game.i18n.localize('carbon2185.Level')} ${pact.level}) `
          + `(${pact.value} ${game.i18n.localize('carbon2185.Slots')})`,
        canCast: canUpcast,
    ...
    const html = await renderTemplate("systems/carbon2185/templates/apps/spell-cast.html", {
      item: item.data,
      canCast: canCast,
      canUpcast: canUpcast,
      spellLevels,
