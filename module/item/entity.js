...
export default class Item5e extends Item {
  ...
  get hasAreaTarget() {
    const target = this.data.data.target;
    return target && (target.type in CONFIG.carbon2185.areaTargetTypes);
  }
  ...
  async _handleResourceConsumption({isCard=false, isAttack=false}={}) {
    ...
    const typeLabel = CONFIG.carbon2185.abilityConsumptionTypes[consume.type];
    ...
    if ( !consume.target ) {
      ui.notifications.warn(game.i18n.format("carbon2185.ConsumeWarningNoResource", {name: this.name, type: typeLabel}));
      return false;
    }
    ...
    if ( [null, undefined].includes(consumed) ) {
      ui.notifications.warn(game.i18n.format("carbon2185.ConsumeWarningNoSource", {name: this.name, type: typeLabel}));
      return false;
    }
    if ( !quantity || (quantity - amount < 0) ) {
      ui.notifications.warn(game.i18n.format("carbon2185.ConsumeWarningNoQuantity", {name: this.name, type: typeLabel}));
      return false;
    }
  ...
  _equipmentChatData(data, labels, props) {
    props.push(
      CONFIG.carbon2185.equipmentTypes[data.armor.type],
      labels.armor || null,
  ...
  _weaponChatData(data, labels, props) {
    props.push(
      CONFIG.carbon2185.weaponTypes[data.weaponType],
    );
  }
  ...
  _consumableChatData(data, labels, props) {
    props.push(
      CONFIG.carbon2185.consumableTypes[data.consumableType],
  ...
  _toolChatData(data, labels, props) {
    props.push(
      CONFIG.carbon2185.abilities[data.ability] || null,
      CONFIG.carbon2185.proficiencyLevels[data.proficient || 0]
    );
  }
  ...
  async rollAttack(options={}) {
    ...
    if ( ["weapon", "spell"].includes(this.data.type) ) {
      if (flags.elvenAccuracy && ["dex", "int", "tech", "people"].includes(this.abilityMod)) {
  ...
  async _rollConsumable(configureDialog) {
    ...
    if ( current <= 0 ) ui.notifications.warn(game.i18n.format("carbon2185.ItemNoUses", {name: this.name}));
  ...
  static async createScrollFromSpell(spell) {
    ...
    // Get scroll data
    const scrollUuid = CONFIG.carbon2185.spellScrollIds[level];
    ...
    const spellScrollData = mergeObject(scrollData, {
      name: `${game.i18n.localize("carbon2185.SpellScroll")}: ${itemData.name}`,
      img: itemData.img,
      data: {
