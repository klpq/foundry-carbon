...
export async function d20Roll({parts=[], data={}, event={}, rollMode=null, template=null, title=null, speaker=null,
  ...
  const _roll = function(parts, adv, form=null) {
    ...
    if ( adv === 1 ) {
      ...
      flavor += ` (${game.i18n.localize("carbon2185.Advantage")})`;
      mods += "kh";
    }
    ...
    // Handle disadvantage
      ...
      nd = 2;
      flavor += ` (${game.i18n.localize("carbon2185.Disadvantage")})`;
      mods += "kl";
    }
    ...
    if ( ability && ability.value ) {
      ...
      if ( abl ) {
        data.mod = abl.mod;
        flavor += ` (${CONFIG.carbon2185.abilities[data.ability]})`;
      }
    }
    ...
    if ( reliableTalent && roll.dice[0].total < 10 ) {
      flavor += ` (${game.i18n.localize("carbon2185.FlagsReliableTalent")})`;
    }
  ...
  return new Promise(resolve => {
    new Dialog({
      ...
      buttons: {
        advantage: {
          label: game.i18n.localize("carbon2185.Advantage"),
          callback: html => roll = _roll(parts, 1, html[0].children[0])
        },
        normal: {
          label: game.i18n.localize("carbon2185.Normal"),
          callback: html => roll = _roll(parts, 0, html[0].children[0])
        },
        disadvantage: {
          label: game.i18n.localize("carbon2185.Disadvantage"),
          callback: html => roll = _roll(parts, -1, html[0].children[0])
        }
      },
      default: "normal",
...
export async function damageRoll({parts, actor, data, event={}, rollMode=null, template, title, speaker, flavor,
  ...
  const _roll = function(parts, crit, form) {
    ...
    if ( crit === true ) {
      ...
      let mult = 2;
      roll.alter(add, mult);
      flavor = `${flavor} (${game.i18n.localize("carbon2185.Critical")})`;
    }
    ...
    new Dialog({
      ...
      buttons: {
        critical: {
          ...
          label: game.i18n.localize("carbon2185.CriticalHit"),
          callback: html => roll = _roll(parts, true, html[0].children[0])
        },
        normal: {
          label: game.i18n.localize(allowCritical ? "carbon2185.Normal" : "carbon2185.Roll"),
